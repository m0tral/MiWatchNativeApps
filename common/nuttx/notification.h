#ifndef XIAOMI_NOTIFICATIONS_H
#define XIAOMI_NOTIFICATIONS_H

/* watchs3 (Xiaomi Watch S3) notification subsystem stub.
 *
 ...
 * subsystem on watchs3. */
typedef enum {
    WATCHS3_NOTIFY_GENERIC   = 0,
    WATCHS3_NOTIFY_CALL      = 1,
    WATCHS3_NOTIFY_SMS       = 2,
    WATCHS3_NOTIFY_ALARM     = 3,
    WATCHS3_NOTIFY_GOAL      = 4,
    WATCHS3_NOTIFY_HEARTRATE = 5,
    WATCHS3_NOTIFY_APP       = 6,
} watchs3_notify_kind_t;

/* Outcome of a post. Negative values are negated errno. */
enum {
    WATCHS3_NOTIFY_OK         =  0,
    WATCHS3_NOTIFY_ERR_FULL   = -1,
    WATCHS3_NOTIFY_ERR_INVAL  = -2,
    WATCHS3_NOTIFY_ERR_NOMEM  = -3,
};
 ...
 * lives in platform/watchs3_platform.h. Returns 0 on success, negative
 * errno on failure. */
typedef int (*notification_insert_t)(const struct notification_msg_t *msg);

#endif /* XIAOMI_NOTIFICATIONS_H */
