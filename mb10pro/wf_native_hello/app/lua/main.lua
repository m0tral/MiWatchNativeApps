...
local FW_TO_APP = {
    ["3.101.043"] = "mb10p_hello_3.101.043.elf",
    ["3.121.043"] = "mb10p_hello_3.101.043.elf",
    ["3.201.016"] = "mb10p_hello_3.201.016.elf",
    ["3.221.016"] = "mb10p_hello_3.201.016.elf",
}
...
local function detect_fw_version()
    ...
    f:close()
    -- Pull just the leading dotted/digit/underscore token ("3.101.043"
    -- or "3_101_043"); the trailing "release-keys" etc. is dropped so the
...
local MODULE_NAME = "mb10p_hello"

local APP_SRC     = SCRIPT_PATH .. APP_NAME
local APP_DST     = "/data/mb10p_com.m0tral.hello.elf"
...
local function make_button(text, y, color, on_clicked)
    ...
    lvgl.Label(button, {
        ...
        text_font = lvgl.Font("MiSans-Demibold", 28, "normal")
    })
...
status = lvgl.Label(root, {
    ...
    text_font = lvgl.Font("MiSans-Demibold", 20, "normal"),
